...
1. インストール後、[*`install.bat`*](#installbat) と同じフォルダー内に生成される [*`gw.setup.bat`*](#gwsetupbat) を、任意のフォルダー内にコピーして実行してください。実行したフォルダーのパスはプロジェクトフォルダーのパスとして各スクリプトや設定ファイル内で使用されます。フォルダーを移動するなどプロジェクトフォルダーのパスに変更がある際は、変更後後に改めて [*`プロジェクトフォルダー\gw.setup.bat`*](#gwsetupbat-1) を実行してください。
1. スクリプトをバンドルする際は [*`プロジェクトフォルダー\gw.setup.bat`*](#gwsetupbat-1) の実行後にプロジェクトフォルダー内に生成される [*`gw.bundle.bat`*](#gwbundlebat) か [*`gw.bundle-watched.bat`*](#gwbundle-watchedbat) を実行します。[*`gw.bundle.bat`*](#gwbundlebat) は実行毎にバンドルを行い、[*`gw.bundle-watched.bat`*](#gwbundle-watchedbat) は一度実行すると終了するまで[*フォルダー `gw`*](#gw) 内にあるファイルが変更される度に自動でバンドルを行い続けます。
...
　バンドルするファイルが入ったフォルダーパスを指定します。この値は [`インストールフォルダー\setup\config.bat`](#setupconfigbat) を通じて動的に定義されます。既定値は `gw` です。

#### placeholder.importFileName
　バンドルするファイルの名前を指定します。この値は [`インストールフォルダー\setup\config.bat`](#setupconfigbat) を通じて動的に定義されます。既定値は [*`gw\index.js`*](#gwindexjs) です。
...
　[*`index.html.template`*](#presetsindexhtmltemplate) から生成される [`index.html`](#indexhtml) の [タグ`<meta>`](https://developer.mozilla.org/ja/docs/Web/HTML/Element/meta) の [属性 `charset`](https://developer.mozilla.org/ja/docs/Web/HTML/Element/meta#charset) の値を示します。生成される [`index.html`](#indexhtml) の文字コードを指定することができます。既定値は `utf-8` です。

#### placeholder["index.html"].lang
　[*`index.html.template`*](#presetsindexhtmltemplate) から生成される [`index.html`](#indexhtml) の [タグ `<html>`](https://developer.mozilla.org/ja/docs/Web/HTML/Element/html) の [属性 `lang`](https://developer.mozilla.org/ja/docs/Web/HTML/Global_attributes/lang) の値を示します。生成される [`index.html`](#indexhtml) の言語を指定することができます。既定値は `ja` です。

#### placeholder["index.html"].title
　[*`index.html.template`*](#presetsindexhtmltemplate) から生成される [`index.html`](#indexhtml) の [タグ `<title>`](https://developer.mozilla.org/ja/docs/Web/HTML/Element/title) の内容を示します。生成される [`index.html`](#indexhtml) のタイトルを指定することができます。既定値は `Hi, world.` です。
...
### gw.setup.bat.preset
　プロジェクトフォルダー内で、バンドルを実行する環境を構築する[バッチファイル](https://ja.wikipedia.org/wiki/%E3%83%90%E3%83%83%E3%83%81%E3%83%95%E3%82%A1%E3%82%A4%E3%83%AB) `gw.setup.bat` の元になるファイルです。`gw.setup.bat` は、[*`install.bat`*](#installbat) の実行時に自動で生成されます。
